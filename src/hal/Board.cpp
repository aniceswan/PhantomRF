...
void Board::setupPins() {
    ...
    for (PinRole r : {PinRole::Nrf24Ce0, PinRole::Nrf24Ce1, PinRole::Nrf24Ce2, PinRole::Nrf24Ce3, PinRole::Nrf24Ce4}) {
        const int8_t p = pin(r);
        if (p >= 0) {
            pinMode(p, OUTPUT);
            digitalWrite(p, LOW);
    ...
    for (PinRole r :
         {PinRole::Nrf24Csn0, PinRole::Nrf24Csn1, PinRole::Nrf24Csn2, PinRole::Nrf24Csn3, PinRole::Nrf24Csn4}) {
        const int8_t p = pin(r);
        if (p >= 0) {
            pinMode(p, OUTPUT);
            digitalWrite(p, HIGH);
